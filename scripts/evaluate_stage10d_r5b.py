...
def run(out,tracked):
 ...
 selected=selectable[0] if selectable else min(results,key=key); q=frames[(selected['regularization_value'],selected['gamma'])]; m=selected['_metrics']['2025']; b=selected['_base']['2025']; improved=[m['NDCG']>b['NDCG']+1e-12,m['actual_top20pct_recall']>b['actual_top20pct_recall']+1e-12,m['within_team_share_Spearman']>b['within_team_share_Spearman']+1e-12,m['player_share_MAE']<b['player_share_MAE']-1e-12]; not_worse=[m['NDCG']>=b['NDCG']-1e-12,m['actual_top20pct_recall']>=b['actual_top20pct_recall']-1e-12,m['within_team_share_Spearman']>=b['within_team_share_Spearman']-1e-12,m['player_share_MAE']<=b['player_share_MAE']+1e-12]; chosen=bool(selected['selectable'] and any(improved) and all(not_worse)); status='B2Z_NS_SELECTED_CHALLENGER' if chosen else 'B2Z_NS_NOT_SELECTED'
