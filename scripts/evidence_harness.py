#!/usr/bin/env python3
"""Generic, fail-closed, resumable execution evidence harness with frozen policy enforcement."""
...
from scripts.evidence_policy import (
    enforce_policy_vs_config,
    generate_manifest,
    resolve_approved_policy,
...
def require_config(config: dict[str, Any], root: Path | None = None) -> list[str]:
    required = {
        "version",
        "stage_id",
        "prompt_path",
    ...
    for protected in config.get("protected_paths", []):
        if not isinstance(protected, (str, dict)):
            errors.append("protected path must be a string or object")
        if isinstance(protected, dict) and "path" not in protected:
            errors.append("protected path object missing path")
    ...
    # Independently anchored policy validation
    check_root = root or Path.cwd()
    stage_id = config.get("stage_id", "")
    anchored_rel, anchored_sha, anchored_policy, anchor_errors = resolve_approved_policy(check_root, stage_id)
    if anchor_errors:
        errors.extend(anchor_errors)
    elif anchored_rel:
        if "policy_path" in config and config["policy_path"] != anchored_rel:
            errors.append(
                f"BLOCKED_BY_POLICY_ANCHOR: stage config policy_path '{config['policy_path']}' "
                f"does not match anchored policy '{anchored_rel}'"
            )
        if anchored_policy:
            weakening_errors = enforce_policy_vs_config(anchored_policy, config)
            for err in weakening_errors:
                errors.append(f"BLOCKED_BY_POLICY_WEAKENING: {err}")
    else:
        if config.get("policy_path"):
            errors.append(
                f"BLOCKED_BY_POLICY_ANCHOR: unapproved stage_id '{stage_id}' cannot specify arbitrary policy_path"
            )
...
def worktree_provenance(root: Path, volatile_paths: list[str]) -> dict[str, Any]:
    ...
    tracked_digest = sha256_text(unstaged)
    staged_digest = sha256_text(staged)
    ...
    return {
        "working_tree_clean": not unstaged and not staged and not entries,
        "working_tree_digest": working_tree_digest,
        "tracked_diff_digest": tracked_digest,
        "staged_diff_digest": staged_digest,
...
def identity(root: Path, config_path: Path, config: dict[str, Any]) -> dict[str, Any]:
    ...
    anchored_rel, anchored_sha, _, _ = resolve_approved_policy(root, stage_id)
    result = {
        ...
        "stage_config_path": str(config_path.relative_to(root)) if config_path.is_relative_to(root) else str(config_path),
        "stage_config_sha256": sha256_file(config_path),
        "policy_path": anchored_rel,
        "policy_sha256": anchored_sha,
        "input_artifacts": config.get("input_artifacts", []),
        "input_sha256": {p: sha256_file(root / p) for p in config.get("input_artifacts", []) if (root / p).exists()},
...
def run_command(command: Any, root: Path, output_dir: Path, command_id: str, meta: dict[str, Any]) -> dict[str, Any]:
    ...
    env = os.environ.copy()
    env.update({
        "EVIDENCE_RUN_ID": meta["run_id"],
        "EVIDENCE_STAGE_ID": meta["stage_id"],
        "EVIDENCE_GIT_COMMIT": meta["git_commit"],
    ...
    ended = utc_now()
    stdout = output_dir / f"{command_id}.stdout.txt"
    stderr = output_dir / f"{command_id}.stderr.txt"
    ...
    if match:
        record["tests_run"] = int(match.group(1))
    return record
...
def _checkpoint(evidence: Path, state: dict[str, Any], kind: str, results: list[dict[str, Any]]) -> None:
    ...
    json_dump(evidence / "execution-state.json", state)
...
def _finalize(root: Path, evidence: Path, config: dict[str, Any], meta: dict[str, Any], state: dict[str, Any]) -> tuple[Path, int]:
    ...
    meta["end_timestamp_utc"] = utc_now()
    json_dump(evidence / "run-identity.json", meta)
    if meta.get("policy_path") and (root / meta["policy_path"]).exists():
        (evidence / "policy.json").write_bytes((root / meta["policy_path"]).read_bytes())
    state["phase"] = "finalized"
    state["finalized"] = True
    cmd_results = json_load(evidence / "command-results.json") if (evidence / "command-results.json").exists() else []
    ...
    json_dump(evidence / "manifest-sha256.json", manifest)
    result = validate(root, evidence)
    json_dump(evidence / "validation.json", result)
...
def run_stage(root: Path, config_path: Path) -> tuple[Path, int]:
    ...
    errors = require_config(config, root)
    if errors:
        raise ValueError("; ".join(errors))
    meta = identity(root, config_path, config)
    evidence = root / config["evidence_root"] / f"{config['stage_id'].lower()}-{meta['run_id']}"
    evidence.mkdir(parents=True, exist_ok=False)
    json_dump(evidence / "run-identity.json", meta)
    (evidence / "stage-config.json").write_bytes(config_path.read_bytes())
    if meta.get("policy_path") and (root / meta["policy_path"]).exists():
        (evidence / "policy.json").write_bytes((root / meta["policy_path"]).read_bytes())
    ...
    state = {
        ...
        "preflight_failures": [f"required protected path missing {path}" for path in missing_required],
        "last_checkpoint_utc": utc_now(),
    }
    _checkpoint(evidence, state, "command", [])
    _checkpoint(evidence, state, "test", [])
...
def resume_stage(root: Path, evidence: Path) -> tuple[Path, int]:
    ...
    state = json_load(evidence / "execution-state.json")
    config_path = root / meta["stage_config_path"]
    prompt_path = root / meta["prompt_path"]
    if git(root, "rev-parse", "HEAD") != meta["git_commit"]:
        raise ValueError("resume rejected: commit changed")
    if not config_path.exists() or sha256_file(config_path) != meta["stage_config_sha256"]:
        raise ValueError("resume rejected: config changed")
    if not prompt_path.exists() or sha256_file(prompt_path) != meta["prompt_sha256"]:
        raise ValueError("resume rejected: prompt changed")
    if meta.get("policy_path"):
        policy_path = root / meta["policy_path"]
        if not policy_path.exists() or sha256_file(policy_path) != meta.get("policy_sha256"):
            raise ValueError("resume rejected: policy changed")
    ...
    for key in ("working_tree_digest", "tracked_diff_digest", "untracked_digest"):
        if current_worktree[key] != meta.get(key):
            raise ValueError(f"resume rejected: worktree drift {key}")
...
def validate(root: Path, evidence: Path, skip_manifest: bool = False, skip_report: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        return {"valid": False, "failures": [f"missing provenance: {exc}"], "status": "BLOCKED"}
    ...
    if not prompt.exists() or meta.get("prompt_sha256") != sha256_file(prompt):
        failures.append("prompt hash mismatch")

    # Policy immutability and superset verification
    policy: dict[str, Any] | None = None
    stage_id = meta.get("stage_id", "")
    anchored_rel, anchored_sha, anchored_policy, anchor_errors = resolve_approved_policy(root, stage_id)
    if anchor_errors:
        failures.extend(anchor_errors)
    elif anchored_rel:
        policy = anchored_policy
        if meta.get("policy_path") != anchored_rel:
            failures.append(
                f"BLOCKED_BY_POLICY_ANCHOR: meta policy_path mismatch (expected {anchored_rel}, got {meta.get('policy_path')})"
            )
        if meta.get("policy_sha256") != anchored_sha:
            failures.append(
                f"BLOCKED_BY_POLICY_ANCHOR: meta policy_sha256 mismatch (expected {anchored_sha}, got {meta.get('policy_sha256')})"
            )
        disk_policy = root / anchored_rel
        if not disk_policy.exists() or sha256_file(disk_policy) != anchored_sha:
            failures.append("BLOCKED_BY_POLICY_MUTATION: disk policy modified or missing")
        evidence_policy = evidence / "policy.json"
        if not evidence_policy.exists() or sha256_file(evidence_policy) != anchored_sha:
            failures.append("BLOCKED_BY_POLICY_MUTATION: evidence policy modified or missing")
        elif policy:
            weakening_errors = enforce_policy_vs_config(policy, config)
            for err in weakening_errors:
                failures.append(f"BLOCKED_BY_POLICY_WEAKENING: {err}")
    ...
    try:
        ...
        if len(commands) != len(config.get("commands", [])) or any(command.get("exit_code") != 0 for command in commands):
            failures.append("stage command failure")
    except (OSError, json.JSONDecodeError):
        commands = []
        failures.append("missing command evidence")

    try:
        tests = json_load(evidence / "test-results.json")
        if len(tests) != len(config.get("test_commands", [])) or any(test.get("exit_code") != 0 for test in tests):
            failures.append("test subprocess failure")
    except (OSError, json.JSONDecodeError):
        tests = []
        failures.append("missing test evidence")
    ...
    for artifact in config.get("required_artifacts", []):
        ...
        if not path.exists():
            if skip_manifest and artifact in ("manifest-sha256.json", "validation.json", "ci-replay-validation.json"):
                continue
            ...
            try:
                ...
                if isinstance(body, dict):
                    if "run_id" in body and body.get("run_id") != meta["run_id"]:
                        failures.append(f"artifact run_id mismatch {artifact}")
                    if "git_commit" in body and body.get("git_commit") != meta["git_commit"]:
                        failures.append(f"artifact commit mismatch {artifact}")
                    if "stage_id" in body and body.get("stage_id") != meta["stage_id"]:
                        failures.append(f"artifact stage_id mismatch {artifact}")
            except (json.JSONDecodeError, AttributeError):
                failures.append(f"invalid provenance artifact {artifact}")
    ...
    try:
        ...
        failures.append("missing protected path evidence")

    for gate in config.get("required_gates", []):
        try:
            if not predicate(locator(json_load(evidence / gate["source_artifact"]), gate["source_locator"]), gate["predicate"]):
                failures.append(f"blocking gate failure {gate['gate_id']}")
        except (OSError, KeyError, IndexError, json.JSONDecodeError):
            failures.append(f"missing gate proof {gate.get('gate_id')}")
    ...
    try:
        ...
        for claim_id in config.get("required_claims", []):
            ...
            if not claim:
                failures.append(f"missing claim proof {claim_id}")
                continue
            ...
            if any(not claim.get(k) for k in mandatory) or claim.get("claim_status") != "PROVEN":
                failures.append(f"missing claim provenance {claim_id}")
                continue
            producer = executed.get(claim["producer_command_id"])
            if not producer:
                failures.append(f"unknown claim producer {claim_id}")
                continue
            if producer.get("exit_code") != 0:
                failures.append(f"failed claim producer {claim_id}")
                continue
            source = evidence / claim["source_artifact"]
            if not source.exists() or sha256_file(source) != claim["source_sha256"]:
                failures.append(f"stale source hash {claim_id}")
                continue
            if claim["run_id"] != meta["run_id"] or claim["git_commit"] != meta["git_commit"]:
                failures.append(f"claim provenance mismatch {claim_id}")
                continue
            ...
            if not declared and not source_bound:
                failures.append(f"unbound claim source {claim_id}")
                continue
            if not predicate(locator(source_body, claim["source_locator"]), claim["predicate"]):
                failures.append(f"claim not proven {claim_id}")
    ...
    # Invariant Proof Validation
    if policy and policy.get("required_test_invariants"):
        proofs_path = evidence / "invariant-proofs.json"
        if not proofs_path.exists():
            failures.append("missing invariant proofs artifact invariant-proofs.json")
        else:
            try:
                proofs_data = json_load(proofs_path)
                failures.extend(validate_invariant_proofs(evidence, proofs_data, meta, policy))
            except (OSError, json.JSONDecodeError) as exc:
                failures.append(f"unreadable invariant proofs: {exc}")

    # Sealed Manifest Verification
    if not skip_manifest:
        manifest_path = evidence / "manifest-sha256.json"
        if manifest_path.exists():
            try:
                manifest_data = json_load(manifest_path)
                failures.extend(validate_manifest_entries(evidence, manifest_data, config, policy))
            except (OSError, json.JSONDecodeError) as exc:
                failures.append(f"BLOCKED_BY_MANIFEST_MISMATCH: unreadable manifest {exc}")
        elif (policy and policy.get("manifest_required")) or "manifest-sha256.json" in config.get("required_artifacts", []):
            failures.append("BLOCKED_BY_MANIFEST_MISMATCH: missing manifest-sha256.json")
    ...
    if not skip_report:
        ...
        if report_path.exists():
            try:
                ...
                if report.get("failure_count") != len(failures):
    ...
    status = "PENDING_INDEPENDENT_REVIEW" if not failures else "BLOCKED"
    return {
        "valid": not failures,
        "failures": failures,
        "status": status,
...
def replay(root: Path, evidence: Path) -> int:
    result = validate(root, evidence)
    json_dump(evidence / "ci-replay-validation.json", result)
