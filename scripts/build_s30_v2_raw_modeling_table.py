...
def load_raw():
    ...
    # ``LTA N`` is the raw OE abbreviation currently supplied for LTA North.
    # Preserve it separately while keeping the project competition identity LCS.
    raw['league_model']=raw.league.replace({'LTA North':'LCS','LTA N':'LCS','LTA':'LCS'})
    raw=raw[(raw.league_model.eq('LCS'))&raw.position.isin(ROLES)&raw.date.notna()].copy()
...
def build():
    ...
    targets=raw.groupby(['prediction_period','player','role','team','league_raw','league_model'],as_index=False).agg(lock_timestamp=('date','min'),realized_fantasy_total=('fantasy_pts','sum'),target_games=('gameid','nunique'))
    for r in targets.itertuples(index=False):
        ...
        records.append({'prediction_period':r.prediction_period,'player':r.player,'role':r.role,'team':r.team,'league_raw':r.league_raw,'league_model':r.league_model,'lock_timestamp':lock.isoformat(),'recent_fantasy_mean_5':float(h.fantasy_pts.mean()) if len(h) else float(role_h.fantasy_pts.mean()),'recent_kills_mean_5':float(h.kills.mean()) if len(h) else float(role_h.kills.mean()),'recent_deaths_mean_5':float(h.deaths.mean()) if len(h) else float(role_h.deaths.mean()),'recent_assists_mean_5':float(h.assists.mean()) if len(h) else float(role_h.assists.mean()),'recent_cs_mean_5':float(h['total cs'].mean()) if len(h) else float(role_h['total cs'].mean()),'recent_games_count':int(len(h)),'realized_fantasy_total':float(r.realized_fantasy_total),'realized_fantasy_target':float(r.realized_fantasy_total/r.target_games),'target_games':int(r.target_games),'feature_source_max_timestamp':h.date.max().isoformat() if len(h) else None,'same_lock_violation':False,'future_violation':False})
...
def main():
    ...
    if OUT.exists() and any(OUT.iterdir()) and not args.replace:raise FileExistsError(OUT)
    if OUT.exists() and any(OUT.iterdir()):
        archive=OUT.with_name('s30_v2_raw_prelock_v2_pre_r2_target_grain_correction')
        if archive.exists():raise FileExistsError(archive)
        shutil.copytree(OUT,archive);shutil.rmtree(OUT)
    elif OUT.exists():
        OUT.rmdir()
    OUT.mkdir(parents=True); table,raw,files=build();table.to_csv(OUT/'modeling_table.csv',index=False,float_format='%.12g')
    dump(OUT/'manifest.json',{'version':'S30_V2_RAW_PRELOCK_V2','league_normalization':{'LTA North':'LCS','LTA N':'LCS','LTA':'LCS'},'raw_league_provenance_fields':['league_raw','league_model'],'raw_sources':[str(p.relative_to(ROOT)) for p in files],'raw_source_sha256':{str(p.relative_to(ROOT)):sha(p) for p in files},'raw_lcs_max_game_timestamp':raw.date.max().isoformat(),'row_count':len(table),'feature_columns':['recent_fantasy_mean_5','recent_kills_mean_5','recent_deaths_mean_5','recent_assists_mean_5','recent_cs_mean_5','recent_games_count'],'target_definition':'arithmetic mean of raw fantasy points across target-period player games; realized_fantasy_total retained for schedule aggregation','target_grain':'player × local prediction period × game-average','cutoff_rule':'all feature rows use game timestamp strictly before target period lock','stage_3e_modified':False})
    print(OUT)
