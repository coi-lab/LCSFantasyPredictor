...
3. Confirm that two main-agent iterations made no progress.
4. Invoke `bounded-debugger` for one persistent failure.
5. Test at most three falsifiable hypotheses with at most two discriminating
   checks per hypothesis.
6. Apply at most one focused correction for a supported hypothesis.
7. Rerun the smallest authoritative check and record the result.
8. Return to `implement-approved-task` only after a verified correction;
